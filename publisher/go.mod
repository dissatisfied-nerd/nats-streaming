module publisher

go 1.21.1

require (
	github.com/dissatisfied-nerd/nats-streaming/internal v0.0.0-20231017163410-8b1a1c6cc7f0 // indirect
	github.com/dissatisfied-nerd/nats-streaming/model v0.0.0-20231017151840-b8f2899aa916 // indirect
	github.com/dissatisfied-nerd/nats-streaming/pkg/checkerror v0.0.0-20231017163951-e4c7909f3b0b // indirect
	github.com/dissatisfied-nerd/nats-streaming/pkg/model v0.0.0-20231017163410-8b1a1c6cc7f0 // indirect
	github.com/gogo/protobuf v1.3.2 // indirect
	github.com/nats-io/nats.go v1.22.1 // indirect
	github.com/nats-io/nkeys v0.3.0 // indirect
	github.com/nats-io/nuid v1.0.1 // indirect
	github.com/nats-io/stan.go v0.10.4 // indirect
	golang.org/x/crypto v0.5.0 // indirect
)
